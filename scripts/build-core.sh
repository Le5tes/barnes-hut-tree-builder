if which emcc ; then
    ...
    emcc --bind lib/c++/src/tree-builder.binder.cpp --pre-js lib/js/pre-build/pre-build.js -s ALLOW_MEMORY_GROWTH=1 -Os -s WASM=1 -s SINGLE_FILE=1 -o lib/js/core/build.js
    BUILD_STATUS=$?
